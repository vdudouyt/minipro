...
void
read_page_ram(minipro_p handle, uint8_t *buf, uint32_t type, const char *name, int size) {
...
void
write_page_ram(minipro_p handle, const uint8_t *buf, uint32_t type, const char *name, int size) {
	int i, blocks_count, addr, len;
	char status_msg[24];
...
void
read_page_file(minipro_p handle, const char *filename, uint32_t type, const char *name, int size) {
...
void
write_page_file(minipro_p handle, const char *filename, uint32_t type, const char *name, int size) {
	FILE *file = fopen(filename, "r");
...
void
read_fuses(minipro_p handle, const char *filename, fuse_decl_t *fuses) {
	int i, d;
...
void
write_fuses(minipro_p handle, const char *filename, fuse_decl_t *fuses) {
...
void
verify_page_file(minipro_p handle, const char *filename, uint32_t type, const char *name, int size) {
	FILE *file = fopen(filename, "r");
...
int main(int argc, char **argv) {
	...
	device_p device = cmdopts.device;
	minipro_p handle = minipro_open(device, cmdopts.icsp);
	...
	if (device->chip_id_bytes_count && device->chip_id) {
		minipro_begin_transaction(handle);
		uint32_t chip_id = minipro_get_chip_id(handle);
		minipro_end_transaction(handle);
